...
func NewLogsTab(client ...*tui.RPCClient) *LogsTab {
	...
	return &LogsTab{client: c}
}

// SetClient 设置 RPC client。
func (t *LogsTab) SetClient(c *tui.RPCClient) { t.client = c }

// Name 返回 Tab 显示名。
func (t *LogsTab) Name() string { return "日志" }

// Init 返回初始 Cmd。
func (t *LogsTab) Init() tea.Cmd { return t.fetch() }
...
func (t *LogsTab) View() string {
	...
	if len(t.entries) == 0 {
		...
		visible := 30
		start := t.scroll
		end := min(start+visible, len(t.entries))
		for i := start; i < end; i++ {
			e := t.entries[i]
			ts := e.Time.Format("15:04:05")
			level := levelStyle(e.Level).Render(fmt.Sprintf("%-5s", e.Level))
			line := fmt.Sprintf("  %s %s %s", timeStyle.Render(ts), level, e.Message)
			if e.Attrs != "" {
		...
		fmt.Fprintf(&b, "\n  %s", tui.StyleHelp.Render(fmt.Sprintf("[%d/%d] ", end, len(t.entries))))
	}
